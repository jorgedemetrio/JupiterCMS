 ...
 */
public interface Pessoa extends JpaRepository<Pessoa, UUID> {
    ...
    @Query("SELECT g FROM Pessoa as g JOIN g.site as s WHERE s.id = :site AND upper(g.titulo) = upper(trim(:titulo + '%'))  AND g.status = 'A' ")
    Page<Pessoa> pegarPorTitulo(@Param("titulo") String titulo, @Param("site") UUID site, Pageable paginacao);
