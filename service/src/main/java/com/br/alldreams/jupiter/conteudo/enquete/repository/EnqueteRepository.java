...
import com.br.alldreams.jupiter.conteudo.enquete.repository.domain.Enquete;
...
public interface EnqueteRepository extends JpaRepository<Enquete, UUID> {
    ...
    @Query("SELECT g FROM Enquete as g JOIN g.site as s WHERE s.id = :site AND upper(g.nome) = upper(trim(:nome + '%'))  AND g.status = 'A' ")
    Page<Enquete> pegarPorNome(@Param("nome") String nome, @Param("site") UUID site, Pageable paginacao);
