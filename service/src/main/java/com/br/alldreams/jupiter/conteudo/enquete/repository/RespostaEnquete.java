 ...
 */
public interface RespostaEnquete extends JpaRepository<RespostaEnquete, UUID> {
    ...
    @Query("SELECT g FROM RespostaEnquete as g JOIN g.site as s WHERE s.id = :site AND upper(g.titulo) = upper(trim(:titulo + '%'))  AND g.status = 'A' ")
    Page<RespostaEnquete> pegarPorTitulo(@Param("titulo") String titulo, @Param("site") UUID site, Pageable paginacao);
