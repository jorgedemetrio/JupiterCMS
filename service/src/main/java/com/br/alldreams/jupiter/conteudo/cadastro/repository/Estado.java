 ...
 */
public interface Estado extends JpaRepository<Estado, UUID> {
    ...
    @Query("SELECT g FROM Estado as g JOIN g.site as s WHERE s.id = :site AND upper(g.titulo) = upper(trim(:titulo + '%'))  AND g.status = 'A' ")
    Page<Estado> pegarPorTitulo(@Param("titulo") String titulo, @Param("site") UUID site, Pageable paginacao);
