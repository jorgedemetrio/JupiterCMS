...
public class Messagem {
    ...
    public <T extends BaseException> T createExcetion(final String chave, final Class<T> tipo, final String... parametros) {
        ...
        if (Objects.nonNull(tipo)) {
            try {
                exception = (tipo.getConstructor(String.class, String.class, String.class).newInstance(mensagem, item.getCodigo(), mensagem));
        ...
        if (Objects.isNull(exception)) {
            throw new RuntimeException("Chave possiovel criar a exception para chave: " + chave + " mensagem: " + mensagem);
        }
        return exception;
    ...
    public BaseException createExcetion(final String chave, final String... parametros) {
        ...
        if (Objects.nonNull(item.getClasse()) && !item.getClasse().isEmpty()) {
            try {
                exception = BaseException.class.cast(Class.forName(item.getClasse()).getConstructor(String.class, String.class, String.class)
