...
import com.br.alldreams.jupiter.conteudo.cadastro.repository.domain.TelefonePessoa;
...
public interface TelefonePessoaRepository extends JpaRepository<TelefonePessoa, UUID> {
    ...
    @Query("SELECT g FROM TelefonePessoa as g JOIN g.site as s WHERE s.id = :site AND upper(g.nome) = upper(trim(:nome + '%'))  AND g.status = 'A' ")
    Page<TelefonePessoa> pegarPorTitulo(@Param("nome") String nome, @Param("site") UUID site, Pageable paginacao);
